...
def run(directory, pattern="*.txt", s_offset=0.0, r_max=None, s_max=None,
        transmitted=False, show=False, save="plots", movie=False):
    """
    ...
    s_max       : float      - keep stations with S <= s_max [mm]
    ...
    df = apply_cuts(dfAll, r_max=r_max, s_max=s_max, transmitted=transmitted)
    optics = compute_optics(df)

    plot_all(optics, dfAll, r_max=r_max, s_max=s_max, show=show, save=save)
...
def main():
    ...
    parser.add_argument("--s-max",       type=float, default=None,
                        help="Keep only stations with S <= s-max [mm] (default: no cut)")
